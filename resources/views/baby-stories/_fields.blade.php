...
<div>
    <x-input-label for="media" :value="$story?->media_path ? 'Replace photo/video (optional)' : 'Photo or video (optional)'" />
    ...
    @if ($story?->media_path)
        <div class="mt-3 flex items-center gap-3">
            @if ($story->isVideo())
                <video src="{{ $story->mediaUrl() }}" class="w-20 h-20 rounded-lg object-cover" muted></video>
            @else
                <img src="{{ $story->mediaUrl() }}" alt="" class="w-20 h-20 rounded-lg object-cover">
            @endif
            <label class="flex items-center gap-2 text-sm text-gray-600">
                <input type="checkbox" name="remove_media" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-600">
                Remove this {{ $story->isVideo() ? 'video' : 'photo' }}
            </label>
        </div>
    @endif
</div>
...
<div>
    ...
    <p class="text-xs text-gray-400 mt-1">A photo/video, a caption, or both — at least one is required.</p>
...
<div>
    ...
    <p class="text-xs text-gray-400 mt-1">Leave blank to auto-detect from a photo's EXIF data, if available, or default to now.</p>
